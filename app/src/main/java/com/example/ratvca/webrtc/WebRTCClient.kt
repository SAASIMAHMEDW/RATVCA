package com.example.ratvca.webrtc
...
class WebRTCClient @Inject constructor(
    ...
    fun rejectCall(target: String) {
        listener?.onTransferEventToSocket(
            DataModel(
                type = DataModelType.CallRejected,
                sender = username,
                target = target,
                data = "Call rejected by $username"
            )
        )
    }

    fun onCallRejected(message: String) {
        listener?.onCallRejected(message)
        closeConnection() // End the call
    }
